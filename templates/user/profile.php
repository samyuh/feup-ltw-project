  ...
  <div class="infoGrid">
    ...
    <section id="favPets">
      ...
      <?php foreach($userPets as $fav) {?>
        <p><a href=../../action/dog_profile.php?idPet=<?=$fav['idPet']?>><?=$fav['petName']?></a></p>
      <?php } ?>
    </section>
    <section id="petsForAdoption">
      <h2>Animais para Adoção</h2>
      <?php foreach($toAdoptPets as $fav3) {?>
        <p><a href=../../action/dog_profile.php?idPet=<?=$fav3['idPet']?>><?=$fav3['petName']?></a></p>
      <?php } ?>
    </section>
    <section id="adopted">
      <h2>Adopted Pets!</h2>
      <?php foreach($adoptPets as $fav2) {?>
        <p><a href=../../action/dog_profile.php?idPet=<?=$fav2['idPet']?>><?=$fav2['petName']?></a></p>
      <?php } ?>
    </section>
  </div>  
</section>